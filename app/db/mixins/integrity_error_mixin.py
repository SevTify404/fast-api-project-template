    ...
    def translate_integrity_error(cls, exception: IntegrityError) -> Optional[str]:
        ...
        try:
            ...
            if constraint_name is None:
                logger.warning(
                    "Nom de la contrainte BD non trouvé dans la cause : %s", cause
                )
                return None

            logger.debug(
                "Nom de la contrainte BD extrait avec succès : %s", constraint_name
            )
            ...
            logger.error(
                "Erreur lors de l'extraction du nom de la contrainte, __cause__ n'est pas dispo : %s",
                exception,
                exc_info=exception,
            )
            return None
            ...
            logger.error(
                "Erreur inattendue lors de la traduction de l'integrity error : %s",
                exception,
                exc_info=exception,
            )
            return None
